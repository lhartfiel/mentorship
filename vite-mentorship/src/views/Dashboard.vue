    ...
    <div class="sessions" v-for="(session, index) in allSessions" :key="`session-${session.id}`">
      <div class="flex justify-between b-shadow  my-6 text-left">
        ...
        <div class="links flex">
          <a href="" @click.prevent="editForm(index, session.dateSessionStart, session.dateSessionEnd, session.name, session.id)" class="px-6 bg-edit flex items-center"><img class="icon " :src=editIcon alt="Edit Icon"></a>
      ...
      <form @submit="editSessionInput" method="post" v-if="editSession && index === editIndex" class="flex flex-wrap content-center">
        ...
        <a href="#" class="block w-full mt-3 mb-6 text-center btn bg-green-600 text-white text-center m-auto p-3" @click.prevent="editSessionInput($event)">Submit</a>
      </form>
    </div>
...
const ADD_SESSION = gql`mutation ($name: String!, $dateSessionStart: DateTime!, $dateSessionEnd: DateTime!) {
  createSessionInput(session: {name: $name, dateSessionStart: $dateSessionStart, dateSessionEnd: $dateSessionEnd}){
  session {
      name
      dateSessionStart
      dateSessionEnd
    ...
    createSessionInput(){
      this.$apollo.mutate({
        ...
        update: (cache, result) => {
          ...
          console.log(result.data.createSessionInput)
    ...
    editSessionInput() {
      this.$apollo.mutate({
        ...
        update: (cache, result) => {
          ...
          const newData = [...data.allSessions]
